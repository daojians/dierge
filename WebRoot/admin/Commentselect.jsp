...
<c:if test="${empty requestScope.LIST }">
	<c:redirect url="Commentservlet"></c:redirect>
</c:if>
<html>
  <head>    
    <title>慕课网后台管理</title>
      ...
      <div>
		  ...
		  <c:forEach items="${requestScope.LIST }" var="item">
		    <tr>
		      ...
		      <td>${item.usersUsername}</td>
		      <td>${item.content}</td>
		      <td>${item.time}</td>
		      <td><button class="layui-btn" onclick="location.href='Commentdeleteservlet?id=${item.id}'">删除</button><td>
